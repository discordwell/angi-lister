...
### Delivery Rate
The "Delivery Rate" KPI is `sent / (sent + failed)` over **non-simulated** messages — only **terminal send outcomes** count. Messages still in flight (`pending`/`generating`) or intentionally not sent by the personalization engine (`declined` by job rules, `skipped` by the LLM) are not delivery failures and are excluded from the denominator; when there are no completed attempts the rate is `None` (shown as `—`, not `0%`). This denominator definition is canonical, shared by `analytics.py::get_tenant_comparison` (admin per-tenant table) and `analytics.py::get_system_health` (the `email_failure_rate_24h` companion). Each page applies its own scope window — the dashboard tile (`metrics.py::get_metrics_summary`) is all-time while the admin table is the last 30 days, exactly as conversion rate does — so the two agree for the same data. Two defects predated this: the dashboard divided by *every* non-simulated message (so a queue backlog, or a tenant that declines/skips most leads, showed a spuriously low rate and a red tile even when every message actually attempted was delivered), and `get_tenant_comparison` counted *simulated* sends in its delivery rate while excluding them from its sibling personalization rate (and the dashboard excluded them) — an internal split. Both are fixed; `tests/test_delivery_rate.py` pins the shared formula, the non-simulated population, and the cross-page agreement.
